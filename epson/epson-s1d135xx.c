...
static int s1d135xx_update_reg(struct s1d135xx *p, uint16_t reg, uint16_t val, const uint32_t bitmask);
static int s1d135xx_set_registers(struct s1d135xx *p, const regSetting_t* map, int n);
static void s1d135xx_cmd(struct s1d135xx *p, uint16_t cmd, const uint16_t *params, size_t n);
static int s1d135xx_set_epd_power(struct s1d135xx *p, int on);
static int s1d135xx_set_power_state(struct s1d135xx *p, enum pl_epdc_power_state state);
...
static int do_fill(struct s1d135xx *p, const struct pl_area *area, unsigned bpp, uint8_t g);
...
static void send_cmd_area(struct s1d135xx *p, uint16_t cmd, uint16_t mode, const struct pl_area *area);
...
static int s1d135xx_read_spi(struct s1d135xx *p, uint8_t *val, size_t size)
{
	...
	for(i=0; i<size; i++)
	{
			...
			p->interface->read_bytes(p->interface, (uint8_t *)&_val, sizeof(uint16_t));
			p->interface->read_bytes(p->interface, (uint8_t *)&_val, sizeof(uint16_t));
			_val = be16toh(_val);
			val[i] = (uint8_t) _val;
	}
...
static int s1d135xx_load_init_code(struct s1d135xx *p, const char *init_code_path)
{
	FILE *init_code_file;
	uint16_t checksum;
	int stat;
...
static int s1d135xx_configure_update(struct s1d135xx *p, int wfid, enum pl_update_mode mode, const struct pl_area *area)
{
	...
	case PL_FULL_UPDATE_NOWAIT:
		cmd->cmd_code = S1D135XX_CMD_UPDATE_FULL; break;
	case PL_PART_UPDATE:
	case PL_PART_UPDATE_NOWAIT:
		cmd->cmd_code = S1D135XX_CMD_UPDATE_PART; break;
	case PL_FULL_AREA_UPDATE:
	case PL_FULL_AREA_UPDATE_NOWAIT:
		cmd->cmd_code = S1D135XX_CMD_UPDATE_FULL_AREA; break;
	case PL_PART_AREA_UPDATE:
	case PL_PART_AREA_UPDATE_NOWAIT:
		cmd->cmd_code = S1D135XX_CMD_UPDATE_PART_AREA; break;
	}
	...
	else if ((mode == PL_FULL_AREA_UPDATE || mode == PL_PART_AREA_UPDATE) && area != NULL)
	{
		cmd->param_count = 5;
		cmd->params[0] = S1D135XX_WF_MODE(wfid);
...
int s1d13524_init_ctlr_mode(struct s1d135xx *p)
{
	...
	p->send_cmd_with_params(p, S1D13524_CMD_INIT_CTLR_MODE, par, ARRAY_SIZE(par));
