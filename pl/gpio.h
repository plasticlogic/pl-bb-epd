...
struct pl_gpio {
	...
	/** Set a GPIO to active state
	    @param[in] gpio GPIO number
	 */
	void (*set_active)(unsigned gpio, uint16_t flags);

	/** Set a GPIO to inactive state
	    @param[in] gpio GPIO number
	 */
...
/** Deconfigure and releases a list of GPIOs
    @param[in] gpio gpio instance
    @param[in] config array of pl_gpio_config structures
    @param[in] n length of the config array
    @return -1 if error, 0 otherwise
*/
int pl_gpio_deconfigure_list(struct pl_gpio *gpio,
			const struct pl_gpio_config *config, size_t n);
